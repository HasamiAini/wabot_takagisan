...
const { help, snk, info, donate, readme, listChannel, infoskinml, textCek, textml, textCdom, textGift, textSquad, textSq, textPembayaran, textPremium, textPubg, textHago} = require('./lib/help')
...
module.exports = Messenger = async (client, message) => {
    ...
    case '!dw' :
         ...
         case '!nulis':
            ...
            break
        case '!premium':
            if (!isOwner) return client.reply(from, 'KHUSUS PRABAYAR wa.me/6283191735552', id)
            if (!isGroupMsg) return client.reply(from, '*Onichan Gomenasai harus di group desu:(..*', id)
            if (!isGroupAdmins) return client.reply(from, '*DASAR MEMBER SOK-SOK MAKE FITUR ADMIN!*', id)
            ...
            if (args[1].toLowerCase() === 'aktif') {
                ...
                client.reply(from, '*oni-chan Sugoi..!!* *premium* perintah berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'nonaktifkan') {
                premium_.splice(chat.id, 1)
                fs.writeFileSync('./lib/premium.json', JSON.stringify(premium_))
                client.reply(from, '*Sumimasen* *premium* perintah berhasil di nonaktifkan di group ini!', id)
            } else {
    ...
    if (!isGroupMsg) return client.reply(from, menuId.textPrem())
            ...
            if (!isGroupAdmins) return client.reply(from, '*DASAR MEMBER SOK-SOK MAKE FITUR ADMIN!*', id)
                    ...
                    case '!nulis3':
                        ...
                        if (args.length === 0) return client.reply(from, `Kirim perintah *#nulis3 contoh : *!nulis3 |nama|kalimat|kelas*`, id)
                        argz = body.trim().split('|')
                        if (argz.length >= 3) {
                            client.reply(from, `Wait sedang diproses...`, id)
                            const nama = argz[1]
                            const teks = argz[2]  
                                    const kelas = argz[3]   
                             if (nama > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            if (kelas > 70) return client.reply(from, '*Maaf max teks 70!!*', id)
                            if (teks > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            client.sendFileFromUrl(from, `https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${teks}&tinta=1`)
                        } else {
            ...
            case '!nulis4':
                if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)  
                        if (args.length === 0) return client.reply(from, `Kirim perintah *#nulis4 contoh : *!nulis4 |nama|kalimat|kelas*`, id)
                        argz = body.trim().split('|')
                        if (argz.length >= 3) {
                            client.reply(from, `Wait sedang diproses...`, id)
                            const nama = argz[1]
                            ...
                            if (kelas > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            if (teks > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            client.sendFileFromUrl(from, `https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${teks}&tinta=2`)
                        } else {
            ...
            case '!nulis5':
                if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)  
                        if (args.length === 0) return client.reply(from, `Kirim perintah *#nulis5 contoh : *!nulis5 |nama|kalimat|kelas*`, id)
                        argz = body.trim().split('|')
                        if (argz.length >= 3) {
                            client.reply(from, `Wait sedang diproses....`, id)
                            const nama = argz[1]
                            ...
                            if (kelas > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            if (teks > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            client.sendFileFromUrl(from, `https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${teks}&tinta=3`)
                        } else {
                ...
                const hasillip = `*[ HASILNYA... ]*\n\n\n*network* : _${res.data.result.card.network}_\n\n*number* : _${res.data.result.card.number}_\n\n*cvv* : _${res.data.result.card.cvv}_\n\n*pin* : _${res.data.result.card.pin}_\n\n*balance* : _${res.data.result.card.balance}_\n\nHacked By : ZeusXz`;
                            CLIENT.sendText(from, hasillip, id)
                            }) 
                    break
                    case '!nickff':
                        if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)  
                        ...
                        const ff = await axios.get('https://api.zeks.xyz/api/nickepep?apikey=apivinz').then(res => {
                            ...
                            client.sendText(from, hasifl, id)
                                }) 
                        break
                    case '!infobmkg':
                        if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)  
                    axios.get(`https://mnazria.herokuapp.com/api/bmkg-gempa`).then (res => {
	                        ...
	                        client.sendText(from, inidia, id)
	                    })
	                            break
                    case '!nulis6':
                        if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)  
                        if (args.length === 0) return client.reply(from, `Kirim perintah #nulis6 contoh : *!nulis6 |nama|kalimat|kelas*`, id)
                        argz = body.trim().split('|')
                        if (argz.length >= 3) {
                            client.reply(from, `Wait sedang diproses....`, id)
                            const nama = argz[1]
                            ...
                            if (kelas > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            if (teks > 70) return client.reply(from, '*Maaf max teks 70!*', id)
                            client.sendFileFromUrl(from, `https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${teks}&tinta=4`)
                        } else {
...
${desc}`)
    ...
    case '!duck':
     ...
     case '!gives':
            ...
            case 'stickdarcula':
                if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)
	       if (args.length === 0) return client.reply(from, 'Teks nya mana??', id)
		 client.reply(from, mess.wait, id)	
	     const darculanyas = `https://carbonnowsh.herokuapp.com/?code=${body.slice(14)}&theme=darcula&backgroundColor=rgba(50, 50, 50, 150)`
         client.sendFileFromUrl(from, darculanyas)
	     break
            case '!tahta':
            case '!harta':
                if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)
             const kalimat = body.slice(7)
             if (!kalimat) return client.reply(from, `Kirim perintah *${prefix}tahta [teks]*\n\nContoh *${prefix}tahta Zeus*`, id)
             if (kalimat.length > 7) return client.reply(from, 'Maksimal 7 Huruf!', id)
             client.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
             await client.sendFileFromUrl(from, `https://api.zeks.xyz/api/hartatahta?text=${kalimat}&apikey=apivinz`,`${kalimat}.jpg`,`Harta Tahta ${kalimat}`, id)        
             break
        ...
        if (!isPremium) return client.reply(from, ' *FITUR KHUSUS PREMIUM..DAFTAR KE wa.me/6283191735552* ', id)               
            client.reply(from, mess.wait, id);
                axios.get('https://akaneko-api.herokuapp.com/api/mobileWallpapers').then(res => {
                    client.sendStickerfromUrl(from, res.data.url,'Desktop Wallpaper.jpeg', id);
                });
             case '!nekonime':
                try {
                    client.reply(from, mess.wait, id)
            ...
            if (respw.data.error) return client.reply(from, respw.data.error, id)
            const anm2 = `➸ Soal : ${respw.data.result.soal_gbr}`
            const jwbanw = `➸ Jawaban : ${respw.data.result.jawaban}\n\n*Thanks Play Game*`
            client.sendFileFromUrl(from, respw.data.result.soal_gbr, 'tebakgambar2.jpg', '_Silahkan Jawab Maksud Dari Gambar Ini_', id)
            client.sendText(from, `Silahka jawab Gambar Ini..\n Waktu Cuman 30Detik\nDimulai Dari Sekarang`, id)
            await sleep(10000)
